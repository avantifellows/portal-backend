...
def get_school_by_name_and_district(name: str, district: str) -> Optional[Dict[str, Any]]:
    """Get school by name and district."""
    return get_school(name=name, district=district)


def get_school_by_name_district_state(name: str, district: str, state: str) -> Optional[Dict[str, Any]]:
...
def get_school(**params) -> Optional[Dict[str, Any]]:
    ...
    query_params = {k: v for k, v in params.items() if v is not None and k in SCHOOL_QUERY_PARAMS}
