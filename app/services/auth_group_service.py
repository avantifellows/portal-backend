...
def get_auth_group(**params) -> Optional[Dict[str, Any]]:
    ...
    query_params = {k: v for k, v in params.items() if v is not None and k in AUTH_GROUP_QUERY_PARAMS}
    ...
    if is_response_valid(response, "Auth Group API could not fetch the data!"):
        auth_group_data = safe_get_first_item(response.json(), "Auth Group does not exist!")
