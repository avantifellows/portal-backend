...
async def user_session(user_session: UserSession):
    ...
    if query_params['user_type'] == 'student':
        user_id_response = student.get_students(build_request(query_params={"student_id": query_params["user_id"]}))

    query_params["user_id"] = user_id_response[0]["user"]["id"]

    session_pk_id_response = await session.get_session(build_request(query_params={"session_id": query_params["session_id"]}))
