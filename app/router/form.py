...
def build_returned_form_schema_data(
    form_schema, number_of_fields_in_form_schema, set_of_fields, priority
):
    """
...
def state_in_returned_form_schema_data(
    form_schema, number_of_fields_in_form_schema, set_of_fields
):
    """
...
def district_in_returned_form_schema_data(
    form_schema, number_of_fields_in_form_schema, set_of_fields, student_data
):
    """
...
def is_user_attribute_empty(set_of_fields, priority, student_data):
    ...
    """
    return set_of_fields[str(priority)]["key"] in mapping.USER_QUERY_PARAMS and (
        student_data["user"][set_of_fields[str(priority)]["key"]] is None
        or student_data["user"][set_of_fields[str(priority)]["key"]] == ""
    )
...
def is_student_attribute_empty(set_of_fields, priority, student_data):
    ...
    return key in mapping.STUDENT_QUERY_PARAMS and (
...
def is_user_or_student_attribute_empty_then_build_schema(
    form_schema, number_of_fields_in_form_schema, set_of_fields, priority, data
):
    """
    ...
    return (
        ...
        if is_user_attribute_empty(set_of_fields, priority, data)
        or is_student_attribute_empty(set_of_fields, priority, data)
...
def is_enrollment_record_attribute_empty(set_of_fields, priority, enrollment_data):
    ...
    """
    return (
        set_of_fields[str(priority)]["key"] == "school_name"
        and enrollment_data["school_id"] is None
        or enrollment_data["school_id"] == ""
    ) or (
        (
            set_of_fields[str(priority)]["key"] in mapping.ENROLLMENT_RECORD_PARAMS
            and set_of_fields[str(priority)]["key"] != "school_name"
            and set_of_fields[str(priority)]["key"] != "student_id"
            and (
                enrollment_data[set_of_fields[str(priority)]["key"]] is None
                or enrollment_data[set_of_fields[str(priority)]["key"]] == ""
            )
        )
    )
...
def is_enrollment_record_attribute_empty_then_build_schema(
    form_schema, number_of_fields_in_form_schema, set_of_fields, priority, data
):
...
def is_student_exam_record_attribute_empty(
    set_of_fields, priority, student_exam_record_data
):
    return (
        set_of_fields[str(priority)]["key"] in mapping.STUDENT_EXAM_RECORD_QUERY_PARAMS
        and set_of_fields[str(priority)]["key"] != "student_id"
        and (
            student_exam_record_data[set_of_fields[str(priority)]["key"]] is None
            or student_exam_record_data[set_of_fields[str(priority)]["key"]] == ""
        )
    )


def is_student_exam_record_attribute_empty_then_build_schema(
    form_schema, number_of_fields_in_form_schema, set_of_fields, priority, data
):
    if set_of_fields[str(priority)]["key"] in mapping.STUDENT_EXAM_RECORD_QUERY_PARAMS:
        if len(data) > 0:
            for record in data:
                if is_student_exam_record_attribute_empty(
                    set_of_fields, priority, record
                ):
                    if set_of_fields[str(priority)]["dependant"]:
                        dependent_hierarchy = [priority]
                        find_dependant_parent(
                            set_of_fields, priority, dependent_hierarchy
                        )
        ...
        return build_returned_form_schema_data(
            form_schema, number_of_fields_in_form_schema, set_of_fields, priority
        )
    return (form_schema, number_of_fields_in_form_schema)
        ...
        if number_of_fields_in_form_schema <= total_number_of_fields:
            ...
            for priority in sorted(priority_order):
                ...
                (
                    returned_form_schema,
                    number_of_fields_in_form_schema,
                ) = is_enrollment_record_attribute_empty_then_build_schema(
                    returned_form_schema,
                    number_of_fields_in_form_schema,
                    fields[str(number)],
                    priority,
