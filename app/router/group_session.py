...
def get_auth_group_details(auth_group_id):
    auth_group_response = requests.get(auth_group_db_url,params={"id": auth_group_id},headers=db_request_token())
    if is_response_valid(auth_group_response, "Auth group API could not fetch the data!"):
        auth_group_data = is_response_empty(auth_group_response.json()[0], True, "Auth group record does not exist!")
        return auth_group_data

        
def get_batch_from_group(group_id):
    response = requests.get(group_db_url,params={"id": group_id},headers=db_request_token())
...
def get_group_for_session(session_id: str):
    response = requests.get(group_session_db_url,params={"session_id": session_id},headers=db_request_token(),)
