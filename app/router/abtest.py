...
def get_ab_test_data(request: Request):
    try:
        ...
        if not file_path.exists():
            raise HTTPException(status_code=404, detail=f"A/B test with ID {id} not found")
        ...
        raise HTTPException(status_code=500, detail=f"Error retrieving A/B test data: {str(e)}")