...
class EnableStudents:
    def __init__(self, data):
        for param in ['grade','date_of_birth','gender','school_name','category','first_name', 'region']:
        ...
        if len(user_already_exists) > 0:
            ...
            if len(student_already_exists) > 0:
                ...
                school_response = school.get_school(
                    build_request(
                        query_params={"name": self.school_name}
                    )
                )
                ...
                if len(school_response) > 0:
                    ...
                    enrollment_record_already_exists = enrollment_record.get_enrollment_records(
                        build_request(
                            query_params={"group_id": school_response[0].id, "group_type":"school", "user_id":student["user"]["id"]}
                        )
                    )
